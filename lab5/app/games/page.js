...
import { useState, useEffect, Suspense, useReducer, useMemo } from "react";
...
function GamesList() {
  ...
  // Inicjalizacja reducera do ulubionych
  ...
  // Monitorowanie stanu uwierzytelnienia użytkownika
  useEffect(() => {
  ...
  const handleToggleSold = async (game) => {
    if (!user) return alert("Musisz być zalogowany, aby kupić grę!");
    try {
  ...
  // 1. Zoptymalizowana tablica kategorii przy użyciu useMemo
  const types = useMemo(() => {
    return [...new Set(games.map((g) => g.category || g.type).filter(Boolean))];
  }, [games]);

  // 2. Zoptymalizowane filtrowanie gier przy użyciu useMemo (+0.5 pkt)
            ...
            >
              ...
              <div className="flex flex-col gap-2 border-t pt-4 mt-4">
                ...
                <button
                  ...
                  className={`w-full py-2 rounded-lg text-white font-bold transition flex items-center justify-center gap-2 ${
                ...
                {/* 2. KUP TERAZ - ZMODYFIKOWANY WARUNEK (Goście i zalogowani kupujący znów go widzą) */}
